...
\centering
\includegraphics[width=0.5\textwidth]{coverage_comparison_dual_axis.png}
\begin{itemize}
    \footnotesize
...
\begin{column}{0.5\textwidth}
    \centering
    \includegraphics[width=0.72\textwidth]{graph_embeddings_pca_tsne_test_plot.png}
\end{column}
\begin{column}{0.5\textwidth}
    \centering
    \includegraphics[width=0.72\textwidth]{PCA_density_plots_manually_cropped.png}
\end{column}
\end{columns}
\vspace{0.3em}
\begin{itemize}
    ...
    \item \textbf{23{,}148} Argoverse graphs identified as coverage gaps, dominated by
    \begin{itemize}
        \item \code{cut\_in} ($\sim$94\% of all Argoverse cut-in instances)
        \item \code{cut\_out} and \code{cut\_out\_intersection}
        \item \code{lead\_neighbor\_opposite\_vehicle\_intersection}
    \end{itemize}
\end{itemize}
\end{frame}
