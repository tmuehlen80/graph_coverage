...
% ---------------------------------------------------------
\begin{frame}{Existing Coverage and Analysis Approaches}
\begin{itemize}
    \item Scenario-based testing is the dominant paradigm (PEGASUS: functional / logical / concrete scenario levels)
    \item \textbf{Approval trap}: statistically demonstrating superior safety needs billions of test km $\rightarrow$ motivates simulation and systematic coverage methods
    \item Standardized coverage criteria: ISO~21448, UL~4600; domain-specific coverage buckets (Foretellix)
    \item Recent trend: represent traffic scenes as \textbf{graphs} for scalable, label-free analysis (semantic scene-graph embeddings, contrastive learning, heterogeneous scenario graphs)
...
\begin{itemize}
    ...
    \begin{itemize}
        \item 250{,}000 scenarios, 11\,s each; tracked trajectories (vehicles, pedestrians, cyclists) + semantic map
...
% ---------------------------------------------------------
\begin{frame}{Defining a Traffic Scene Graph}
\begin{itemize}
    \item \textbf{Map graph} $G_{\text{map}}$: directed multigraph, nodes = lane segments (geometry + semantics)
    \begin{itemize}
        \item edges: \textbf{following}, \textbf{neighbor}, \textbf{opposite}
    \end{itemize}
    \item \textbf{Actor graph} $G_{\text{actor}}$: dynamic relationships between actors at one timestep
    \begin{itemize}
        \item nodes: primary/occupied lane IDs, position $s$, 3D position, speed, actor type, lane-change indicator
        \item edges: actor--actor relation type + path length along the lane network
    \end{itemize}
    \item Three relation types, ordered by semantic hierarchy: \textbf{Following/Leading}, \textbf{Neighbor}, \textbf{Opposite}
    \item Lane-change indicator ($\Delta t = 1$\,s) already captures cut-in/cut-out within a single-timestep graph
\end{itemize}
\end{frame}

% ---------------------------------------------------------
\begin{frame}{Hierarchical Graph Construction Algorithm}
\begin{itemize}
    \item \textbf{Phase 1 -- Relation Discovery}: for each actor pair, find a connecting path in the map graph; path structure determines relation type; filtered by distance/path-length thresholds
...
\end{table}
\vspace{-0.3em}
{\small Redundancy pruning reduces edges while preserving connectivity, keeping isomorphism checks tractable in dense traffic.}
\end{frame}
...
\begin{itemize}
    ...
    \item Strategy: (1) define archetype set $S$; (2) fix node/edge attributes considered; (3) match each scene graph against $S$ $\rightarrow$ coverage table $C$
...
\begin{itemize}
    ...
    \item CARLA peaks at higher PC1 values; Argoverse peaks at negative PC1 values $\rightarrow$ datasets occupy different regions of the embedding space
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item Scales across scenarios without scenario-specific handling; naturally accommodates varying numbers of actors
    \item \textbf{Future work}: multi-timestep temporal graphs; automatically extracting new archetype candidates from dense, unmatched embedding regions
