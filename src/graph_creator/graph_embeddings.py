...
class TrainableGraphGINE(torch.nn.Module):
    def __init__(self, node_features, edge_features, embedding_dim=128, hidden_dim=64, num_layers=3, num_classes=None):
        ...
        # IMPROVED: Projection head for contrastive learning - keep same dimension
        self.projection_head = torch.nn.Sequential(
            torch.nn.Linear(embedding_dim, embedding_dim),
            torch.nn.ReLU(),
            torch.nn.Linear(embedding_dim, embedding_dim)  # Changed from embedding_dim // 2
        )
    ...
    def forward(self, data):
        ...
        # IMPROVED: L2-normalize embeddings for better contrastive learning
...
# IMPROVED: Enhanced data augmentation for contrastive learning
def augment_graph(data, node_noise=0.1, edge_noise=0.1, drop_edge_prob=0.0):
    """
    Enhanced graph augmentation with multiple strategies

    Args:
        data: PyG Data object
        node_noise: Std of Gaussian noise added to continuous node features
        edge_noise: Std of Gaussian noise added to continuous edge features
    ...
    # NEW: Random edge dropping for more diverse augmentations
    if drop_edge_prob > 0 and augmented_data.edge_index.size(1) > 0:
        edge_mask = torch.rand(augmented_data.edge_index.size(1), device=data.edge_index.device) > drop_edge_prob
