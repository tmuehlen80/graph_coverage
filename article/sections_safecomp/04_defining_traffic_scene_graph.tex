...
\begin{table}[ht]
      \centering
      \caption{Input parameters for actor graph construction, per relation type. Max.\ distance is the Euclidean limit used in the discovery phase (forward/backward); max.\ path length bounds the number of map-graph edges in a connecting path during construction. The timestep increment is $\Delta t = 1.0$\,s.}
