...
\subsection{Datasets}
\textbf{Argoverse 2.0} \cite{Argoverse2} is a large-scale dataset of 250,000 scenarios (11\,s each) with tracked trajectories for vehicles, pedestrians, and cyclists, plus semantic map information (lane boundaries, crosswalks, traffic signals) across diverse geographic locations. A subset of 17,000 training scenes is used.
...
This is especially beneficial in dense traffic, where the number of pairwise relations grows quadratically with the number of actors: pruning the redundant edges keeps both the graph and the subsequent isomorphism checks tractable, while the complex multi-actor interaction remains fully represented. Where an application nonetheless requires every pairwise relation to be explicit, the pruning can be gated by a lower bound on the implicit path length (equivalently, a local-density threshold): edges are then removed only above this bound, so that the closest and most safety-relevant relations in dense traffic are always retained.
