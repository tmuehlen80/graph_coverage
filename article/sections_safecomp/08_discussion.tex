...
\begin{table}[ht]
\centering
\caption{The 18 subgraph archetypes grouped by complexity. Edge types: \textit{fl}~=~following\_lead, \textit{lv}~=~leading\_vehicle, \textit{nv}~=~neighbor\_vehicle, \textit{ov}~=~opposite\_vehicle.}
\label{tab:subgraph_archetypes}
\begin{tabular}{lcp{6.5cm}}
\toprule
\textbf{Group} & \textbf{Count} & \textbf{Examples} \\
\midrule
Simple (2-actor)   & 3 & Following, opposite, neighbor \\
Complex (3-actor)  & 8 & Cut-in, platoon, opposite traffic, lead+neighbor (with intersection variants) \\
Complex (4-actor)  & 5 & Cut-out, 4-vehicle platoon, lead+follow+opposite (with intersection variants) \\
Complex (5-actor)  & 2 & Lead+neighbor+opposite (with/without intersection) \\
\bottomrule
\end{tabular}
\end{table}
...
The identified coverage gaps in CARLA are substantial. The most critical gaps appear in intersection-related archetypes: \texttt{cut\_out\_intersection} shows nearly complete absence in CARLA (less than 1\% coverage) while appearing in approximately 8\% of Argoverse scenes. Similarly, \texttt{lead\_neighbor\_opposite\_vehicle\_intersection} appears in roughly 6\% of Argoverse data but is virtually absent in CARLA. The \texttt{cut\_in\_intersection} pattern exhibits a comparable gap of approximately 7 percentage points. Both \texttt{cut\_in} and \texttt{cut\_out} scenarios show significant underrepresentation in CARLA regardless of intersections, indicating that the traffic generation script lacks the dynamic lateral maneuvers characteristic of real-world driving.
...
\begin{figure}[ht!]
   ...
   \caption{Role-specific speed distribution comparison for the lead vehicle in front with neighbor vehicle scenario. Green rectangles mark identified coverage gaps where Argoverse shows sufficient density but CARLA is nearly empty. Role~``a'' = ego vehicle, role~``b'' = lead vehicle, role~``c'' = neighbor vehicle.}
...
The GINE model (Section~\ref{chapter:implementation_of_graph_embeddings_for_traffic_scene_analysis}) is trained jointly on CARLA and Argoverse. As a plausibility check, nearest neighbours in embedding space consistently share actor configuration and road geometry with the query (Fig.~\ref{fig:plausibility_checks}), confirming the model captures scene structure rather than superficial features.
...
\textbf{Coverage Gap Identification in Embedding Space:} To quantify coverage gaps, Argoverse graphs whose subgraph isomorphism labels indicate under-represented scenario types are projected into the joint embedding space and compared against the CARLA density. 23,148 Argoverse graphs are identified as coverage gaps---dominated by \texttt{cut\_in} (${\sim}94\%$ of all Argoverse cut-in instances), \texttt{cut\_out}, \texttt{cut\_out\_intersection}, and \texttt{lead\_neighbor\_opposite\_vehicle\_intersection}---clustering in embedding regions where CARLA density is negligible (Fig.~\ref{fig:embedding_coverage_gaps}), directly confirming and spatially localising the gaps found by subgraph isomorphism.
