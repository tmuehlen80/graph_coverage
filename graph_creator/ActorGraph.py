...
class ActorGraph:
    ...
    def _has_path_within_distance(self, G, source, target, max_distance):
        """
        Check if there's a path from source to target with at most max_distance edges.
        
        Args:
            G: NetworkX graph
            source: Source node
            target: Target node
            max_distance: Maximum number of edges in the path
            
        Returns:
            True if there's a path with at most max_distance edges, False otherwise
        """
        if source == target:
            return True
        ...
        try:
            ...
            # len(path) - 1 is the number of edges
        ...
        if nx.has_path(G_map.graph, primary_lane_A, primary_lane_B):
            ...
            if len(path) > 1 and all(
                ...
                if path_length < max_distance_lead_veh_m:
                    ...
                    # Lane-based path_length can be misleading on curved roads
                    ...
                    # Only create following relation if Euclidean distance is also within limit
                    if euclidean_dist < max_distance_lead_veh_m:
                        return ("following_lead", path_length)
            ...
            if (
                ...
                if path_length < max_distance_neighbor_forward_m:
                    ...
                    # Only create neighbor relation if Euclidean distance is also within limit
                    if euclidean_dist < max_distance_neighbor_forward_m:
    ...
    def _add_opposite_edges(self, G_t, relations_dict, max_node_distance, max_distance_opposite_forward_m, max_distance_opposite_backward_m):
        ...
        for actor_id, target_actor_id, path_length in opposite_relations:
            ...
            has_any_path = has_short_path_A_to_B or has_short_path_B_to_A
            should_add = (not has_any_path and abs(path_length) <= max_distance)
            
            should_add_A_to_B = should_add
            should_add_B_to_A = should_add
